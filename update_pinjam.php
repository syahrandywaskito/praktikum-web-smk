...
<html lang="en">
  <head>
    ...
    <title>Update Data Peminjaman Buku</title>
  ...
  <body>
	...
	<section class="form-container">
		<div class="container center">
			...
			$query = mysqli_query($koneksi, "SELECT * FROM peminjaman WHERE kode_pinjam = '$kode'");
			$row = mysqli_fetch_array($query); 
			 ?>
			<form method="post" action="proses_update_pinjam.php">
        <div class="mb-3">
          <label class="form-label">Kode Peminjaman</label>
          <input type="text" class="form-control" readonly value="<?php echo $row['kode_pinjam']; ?>" name="kode_pinjam">
        </div>
        <div class="mb-3">
          ...
          <select name="NIS" class="form-control">
            <?php
              ...
              foreach ($NIS as $baris) {
                 echo "<option value=" . $baris['NIS'] . ">" . $baris['NIS'] . " - " . $baris['nama'] . "</option>";
               } 
              echo "<option>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~</option>";
             ?>
          </select>
        ...
        <div class="mb-3">
          ...
          <select name="kode_buku" class="form-control">
            <?php
              ...
              echo "<option>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~</option>";
              include "koneksi.php";
              $kode_buku = mysqli_query($koneksi, "SELECT * FROM buku");
              foreach ($kode_buku as $baris) {
                 echo "<option value=" . $baris['kode_buku'] . ">" . $baris['kode_buku'] . " - " . $baris['judul_buku'] . "</option>";
               }
              echo "<option>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~</option>"; 
             ?>
          </select>
        ...
        <div class="mb-3">
          ...
          <input type="date" class="form-control" value="<?php echo $row['tanggal_pinjam'];?>" name="tanggal_pinjam">
        </div>
        <div class="mb-3">
          <label class="form-label">Tanggal Kembali</label>
          <input type="date" class="form-control" value="<?php echo $row['tanggal_kembali'];?>" name="tanggal_kembali">
        </div>
